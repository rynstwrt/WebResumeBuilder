...
<!--    {/snippet}-->
<!--    <span class="font-medium">Successfully added work experience!</span>-->
<!--</Alert>-->

<!--<Toast>-->
<!--    {#snippet icon()}-->
<!--        <InfoCircleOutline class="text-primary-500 bg-primary-100 dark:bg-primary-800 dark:text-primary-200 h-6 w-6"/>-->
<!--    {/snippet}-->
<!--    Set yourself free.-->
<!--</Toast>-->
...
<ToastContainer position="top-right">
    {#each toasts as toast (toast.id)}
        <Toast color={toast.color} dismissable={true}
               params={{ delay: 100, duration: 500 }} class="w-64"
               onclose={handleClose(toast.id)} bind:toastStatus={toast.visible}>
            {#snippet icon()}
                <CheckCircleSolid class="h-5 w-5"/>
                <span class="sr-only">Check icon</span>
            {/snippet}
            <!--{toast.message}-->
            <span class="text-gray-300 font-light">{toast.message}</span>
        </Toast>
    {/each}
</ToastContainer>
